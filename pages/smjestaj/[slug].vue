...
export default defineComponent({
  setup() {
    ...
    // Get main image URL - use thumbnail as the main image
    ...
    // Get gallery images from slike_smjestaj
    const getGalleryImages = (): string[] => {
      const images: string[] = [];

      // Check for slike_smjestaj property
      if (
        ...
        currentCompleteSmjestaj.value.slike_smjestaj.forEach(
          (slikaItem: any) => {
            // The structure shows slika_smjestaj is the ID of the image
      ...
      // If there are no images, use default placeholder
      if (images.length === 0) {
    ...
    // Fetch accommodations from the same city
    const fetchCityAccommodations = async (city: string, limit: number = 3) => {
      ...
      try {
        ...
        // Filter out the current accommodation
        citySmjestaji.value = results
          .filter(
            (smjestaj) => smjestaj.id !== currentCompleteSmjestaj.value?.id
          )
          .slice(0, limit);
      } catch (err) {
    ...
    // Navigate to accommodation detail
    const goToDetail = (smjestaj: Smjestaj) => {
      // Preserve query parameters from current route
    ...
    onMounted(async () => {
      if (slug.value) {
        ...
        // After main accommodation is loaded, fetch accommodations from the same city
